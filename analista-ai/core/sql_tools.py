...
def _store_image(
    session_id: str, image_base64: str, title: str, chart_type: str
) -> str:
    """
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            for table_name in [t[0] for t in tables]:
                ...
                for col in columns:
                    ...
                    if any(
                        keyword in col_name
                        for keyword in ["año", "year", "fecha", "date", "time"]
                    ):
                        try:
                            cursor.execute(
                                f"SELECT DISTINCT {col[1]} FROM {table_name} ORDER BY {col[1]} LIMIT 10"
                            )
                            values = cursor.fetchall()
                            if values:
                                date_patterns.append(
                                    f"  • {table_name}.{col[1]}: {[v[0] for v in values[:5]]}..."
                                )
                        except:
                            pass
                ...
                if total_records > 0:
                    ...
                    for col in columns:
                        if col[2].upper() in [
                            ...
                            try:
                                cursor.execute(
                                    f"SELECT COUNT(DISTINCT {col[1]}) as unique_count FROM {table_name}"
                                )
                                ...
                                if unique_count <= min(20, total_records * 0.5):
                                    cursor.execute(
                                        f"SELECT DISTINCT {col[1]} FROM {table_name} LIMIT 5"
                                    )
                                    sample_values = [v[0] for v in cursor.fetchall()]
                                    categorical_patterns.append(
                                        f"  • {table_name}.{col[1]} ({unique_count} valores): {sample_values}..."
                                    )
                            except:
                                pass
            ...
            for table_name in [t[0] for t in tables]:
                ...
                for col in columns:
                    if col[2].upper() in [
                        ...
                        try:
                            cursor.execute(
                                f"SELECT MIN({col[1]}), MAX({col[1]}), COUNT(*) FROM {table_name} WHERE {col[1]} IS NOT NULL"
                            )
                            min_val, max_val, count = cursor.fetchone()
                            if count > 0:
                                numeric_patterns.append(
                                    f"  • {table_name}.{col[1]}: rango [{min_val}, {max_val}] ({count} valores)"
                                )
                        except:
                            pass
            ...
            schema_info += "-" * 40 + "\n"
            schema_info += (
                "• Explora los datos: SELECT * FROM [nombre_tabla] LIMIT 10\n"
            )
            schema_info += "• Cuenta registros: SELECT COUNT(*) FROM [nombre_tabla]\n"
            schema_info += (
                "• Valores únicos: SELECT DISTINCT [columna] FROM [nombre_tabla]\n"
            )
            schema_info += "• Agrupaciones: SELECT [columna], COUNT(*) FROM [nombre_tabla] GROUP BY [columna]\n"
            schema_info += (
                "• Unir tablas: usa las claves foráneas detectadas arriba para JOIN\n"
            )
            ...
            schema_info += (
                "2. Identifica las relaciones entre tablas usando las claves foráneas\n"
            )
            schema_info += "3. Construye consultas JOIN según necesites\n"
            schema_info += (
                "4. Usa columnas temporales y categóricas para filtros específicos\n"
            )
...
def analyze_data_pandas(query: str) -> str:
    ...
    try:
        ...
        analysis = "📊 ANÁLISIS ESTADÍSTICO COMPLETO\n"
        ...
        analysis += (
            f"  • Forma de los datos: {df.shape[0]:,} filas × {df.shape[1]} columnas\n"
        )
        analysis += f"  • Columnas: {list(df.columns)}\n"
        analysis += (
            f"  • Memoria utilizada: {df.memory_usage(deep=True).sum():,} bytes\n\n"
        )

        # Tipos de datos
        analysis += "🏷️ TIPOS DE DATOS:\n"
        ...
        if len(categorical_cols) > 0:
            ...
            for col in categorical_cols:
                ...
                if unique_count <= 20:
                    ...
                    analysis += "    Distribución:\n"
                    for value, count in value_counts.head(10).items():
                        percentage = (count / len(df)) * 100
                        analysis += f"      - {value}: {count:,} ({percentage:.1f}%)\n"
                    if len(value_counts) > 10:
                        analysis += (
                            f"      - ... y {len(value_counts) - 10} valores más\n"
                        )
            analysis += "\n"
        ...
        if missing.any():
            ...
            for col, count in missing.items():
                if count > 0:
                    percentage = (count / len(df)) * 100
                    analysis += (
                        f"  • {col}: {count:,} valores faltantes ({percentage:.1f}%)\n"
                    )
            analysis += "\n"
        else:
    ...
    try:
        ...
        if chart_type.lower() == "bar":
            ...
            if y_column and y_column in df.columns:
                ...
                labels = (
                    df[x_column] if x_column and x_column in df.columns else df.index
                )
        ...
        image_id = _store_image(
            current_session_id,
            f"data:image/png;base64,{img_base64}",
            title or f"Gráfica {chart_type}",
            chart_type,
        )

        # Retornar solo mensaje corto (token-eficiente)
        return (
            f"✅ Gráfica {chart_type} creada exitosamente: '{title}' (ID: {image_id})"
        )
    ...
    try:
        if citation_style.lower() == "apa":
            ...
            if len(parts) >= 2:
                ...
                return (
                    f"Fuente: {source_name}. {url}" if url else f"Fuente: {source_name}"
                )
            else:
                # Solo un elemento
                if "http" in source_info:
...
def create_sources_section(sources_list: str, include_title: bool = True) -> str:
    ...
    try:
        sources = [
            source.strip() for source in sources_list.split(";") if source.strip()
        ]
...
@tool
def create_complete_references_section(
    web_sources: str = "", include_rag: bool = True
) -> str:
    """
    Crea una sección completa de referencias que incluye tanto fuentes web como documentos RAG.

    Esta herramienta unifica todas las referencias en un formato profesional, incluyendo:
    - Fuentes web consultadas (si se proporcionan)
    - Documentos especializados RAG utilizados en la sesión (si están disponibles)

    Args:
        web_sources: Lista de fuentes web separadas por punto y coma (opcional)
                    Formato: "fuente1_info; fuente2_info; ..."
        include_rag: Si incluir las referencias RAG registradas automáticamente (por defecto True)

    Returns:
        Sección completa de referencias en Markdown profesional

    Ejemplo de uso:
        create_complete_references_section("MinSalud Colombia, 2024, Políticas alimentarias, https://url1; FAO, 2023, Estadísticas, https://url2")
        create_complete_references_section()  # Solo fuentes RAG
    """
    try:
        references_sections = []

        # 1. Agregar fuentes web si se proporcionan
        if web_sources and web_sources.strip():
            web_section = create_sources_section(
                web_sources.strip(), include_title=False
            )
            if web_section:
        ...
        # 2. Agregar fuentes RAG si están habilitadas
        if include_rag:
            try:
                from .rag_tools import _rag_source_tracker

                session_id = get_current_session_id()

                if session_id:
                    rag_section = _rag_source_tracker.format_rag_sources(session_id)
                    if rag_section:
                        # Remover el título ya que lo manejaremos aquí
                        rag_content = rag_section.replace(
                            "## 📚 Referencias RAG - Documentos Especializados\n\n", ""
                        )
                        references_sections.append(
                            ("## 📚 Documentos Especializados (RAG)\n\n", rag_content)
                        )

            except ImportError:
                pass  # RAG no disponible
            except Exception:
                pass  # Error al obtener fuentes RAG

        # 3. Construir la sección final
        if not references_sections:
            return ""

        final_section = "\n---\n\n# 📖 Referencias y Fuentes\n\n"

        for title, content in references_sections:
            final_section += title + content + "\n"

        # Agregar nota final
        final_section += "\n---\n"
        final_section += (
            "*Esta sección incluye todas las fuentes consultadas para el análisis: "
        )
        final_section += "datos locales procesados de la base de datos, fuentes web para contexto adicional "
        final_section += "y documentos especializados de la base de conocimientos.*\n"

        return final_section

    except Exception as e:
        return f"❌ Error creando sección de referencias: {e}"
